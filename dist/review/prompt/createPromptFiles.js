...
exports.createPromptFiles = void 0;
const getChangedIndicesAndLength = (contentLines, changedLinesArray) => {
    const changedIndices = {};
    let totalChangedLinesLength = 0;
    let minIndex = Infinity;
    let maxIndex = -Infinity;
    const trimmedContentLinesMap = new Map();
...
const calculateStartAndEnd = (minIndex, maxIndex, contentLinesLength, maxSurroundingLines) => {
...
const expandRange = (start, end, contentLines, remainingSpace) => {
...
const createPromptContent = (start, end, changedIndices, contentLines) => {
...
const createPromptFiles = (files, maxPromptPayloadLength, maxSurroundingLines) => {
    return files.reduce((result, file) => {
        const contentLines = file.fileContent.split("\n");
        const changedLinesArray = file.changedLines.split("\n");
        const { changedIndices, totalChangedLinesLength, minIndex, maxIndex } = getChangedIndicesAndLength(contentLines, changedLinesArray);
...
exports.createPromptFiles = createPromptFiles;
